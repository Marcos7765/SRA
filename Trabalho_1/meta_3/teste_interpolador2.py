...
while (t := sim.getSimulationTime()) < 60:
    ...
    v_esq_list.append(velocidades[0])
    v_dir_list.append(velocidades[1])

    # Armazena posição do robô
    lx.append(pos_robo[0])
    ly.append(pos_robo[1])
...
plt.title("Velocidades das rodas ao longo do tempo")
plt.plot(v_esq_list, label='Roda Esquerda (v_esq)')
plt.plot(v_dir_list, label='Roda Direita (v_dir)')
